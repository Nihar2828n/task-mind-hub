    ...
    <div className="px-4 pt-2 pb-24 max-w-lg mx-auto space-y-5">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex items-center gap-4">
        ...
        <div className="flex-1 space-y-2">
          ...
          <button onClick={() => setShowMood(true)} className="glass-card p-3 flex items-center justify-between w-full hover:bg-muted/50 transition-colors">
            <span className="text-xs text-muted-foreground">Mood</span>
            <span className="text-lg">{todayMood ? moodEmojis[todayMood.mood - 1] : <Smile size={18} className="text-muted-foreground" />}</span>
          </button>
        </div>
      </motion.div>

      {/* Fixed Routine */}
      {settings.showFixedRoutine && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
          <h2 className="text-sm font-semibold text-foreground mb-2">Daily Routine</h2>
          <RoutineTimeline />
        </motion.div>
      )}
